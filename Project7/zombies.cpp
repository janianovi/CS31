...
public:
    ...
    bool Edged(int dir); //helper function
...
bool Zombie::Edged(int dir) //helper funtion to determine if zombie is on edge
{
    switch (dir) {
        case 0:
            if (m_row == 1)
...
void Player::moveOrAttack(int dir)
{
    ...
    // dir, attack it.  Otherwise, move the player to that position if possible
    int trow = m_row;
    int tcol = m_col;
    
    if (m_arena->determineNewPosition(trow, tcol, dir))
    {
        if (m_arena->numZombiesAt(trow, tcol) < 1)
        {
            m_row = trow;
            m_col = tcol;
        }
        ...
        {
            m_arena->attackZombieAt(trow, tcol, dir);
        }
    }
}
...
{
    //move in direction dir and return false if cannot move
    switch (dir)
    {
        case UP:
            if (r <= 1)
...
bool Arena::attackZombieAt(int r, int c, int dir) //attacks a zombie
{
    for (int i = 0; i < m_nZombies; i++)
    {
        if (m_zombies[i]->row() == r && m_zombies[i]->col() == c)
        {
            if (m_zombies[i]->getAttacked(dir)) //removes zombie if attacked before
            {
                delete m_zombies[i];
...
bool Arena::moveZombies() //moves a zombie
{
